...
# --- File Paths ---
# Files are assumed to be in the same directory as app.py in the GitHub repository
model_path = 'gradient_boosting_model.pkl'
features_path = 'gradient_boosting_features.pkl'
label_encoder_target_path = 'product_category_label_encoder.pkl'
data_path = 'data.csv'  # Assumes data.csv is in the same directory as app.py
...
try:
    ...
    st.error(f"Data file 'data.csv' not found. Ensure it is uploaded to the GitHub repository or update the path.")
    st.stop()
except KeyError as e:
    st.error(f"Missing expected column in data file: {e}")
    st.stop()
except Exception as e:
...
if not os.path.exists(model_path):
    st.error(f"Model file '{model_path}' not found. Ensure it is uploaded to the GitHub repository.")
    st.stop()
try:
...
if not os.path.exists(features_path):
    st.error(f"Feature columns file '{features_path}' not found. Ensure it is uploaded to the GitHub repository.")
    st.stop()
try:
...
st.header("Enter Details to Predict Suggested Product Category")

# --- Input Widgets ---
st.subheader("Customer and Transaction Details")
...
# Numerical inputs
age = st.number_input("Age", min_value=0, max_value=120, value=30, step=1, help="Enter the customer's age")
total_bill = st.number_input("Total Bill", min_value=0.0, value=100.0, step=0.1, help="Enter the total bill amount")
transaction_qty = st.number_input("Transaction Quantity", min_value=1, value=1, step=1, help="Enter the number of items")
...
if st.button("Predict Product Category"):
    ...
    try:
        ...
        st.write("Please check input values and ensure all model files are correctly loaded.")

# --- Instructions ---
st.markdown("---")
st.markdown("**How to Run This App:**")
st.code("1. Save this code as `app.py`")
st.code("2. Ensure 'gradient_boosting_model.pkl', 'gradient_boosting_features.pkl', 'product_category_label_encoder.pkl', and 'data.csv' are in the same GitHub repository.")
st.code("3. Install dependencies: `pip install streamlit pandas scikit-learn joblib`")
st.code("4. For local running, execute: `streamlit run app.py`")
st.code("5. For Streamlit Cloud, deploy via your GitHub repository with all files.")
st.markdown("If hosted on Streamlit Cloud, ensure all files are uploaded to your GitHub repository.")
